// src/middlewares/role/validateInput.js

/**
 * Validation middleware specifically for Role management operations
 */
...
const createValidator = (validationRules) => {
  return (req, res, next) => {
    ...
    // Normalize data - trim strings if they exist in body
...
export const validateRoleCreate = createValidator({
...
export const validateRoleSearch = createValidator({
...
export const validateRoleId = (req, res, next) => {
  ...
  if (!id || id <= 0 || !Number.isInteger(id)) {
    return res.status(400).json({
      error: 'Validation failed',
      details: { id: 'Role ID must be a positive integer' }
    });
  }
  req.params.id = id;
  next();
