...
export async function createPermission(name, active = true) {
  ...
  const rows = await sequelize.query(SQL.PERMISSION.PERM_CREATE, {
    replacements: params,
...
export async function getPermissionById(id) {
  const rows = await sequelize.query(SQL.PERMISSION.PERM_GET_BY_ID, {
...
export async function getPermissionByName(name) {
  const rows = await sequelize.query(SQL.PERMISSION.PERM_GET_BY_NAME, {
    replacements: { name },
    type: QueryTypes.SELECT
  });
  return getFirstResult(rows);
...
export async function listPermissions(active, search, page = 1, pageSize = 50) {
  ...
  try {
    const rows = await sequelize.query(SQL.PERMISSION.PERM_LIST, {
...
export async function updatePermission(id, name, active) {
  ...
  const rows = await sequelize.query(SQL.PERMISSION.PERM_UPDATE, {
    replacements: params,
    type: QueryTypes.SELECT
  });
  return getFirstResult(rows);
...
export async function activatePermission(id) {
  const rows = await sequelize.query(SQL.PERMISSION.PERM_ACTIVATE, {
    replacements: { id: Number(id) },
    type: QueryTypes.SELECT
  });
  return getFirstResult(rows);
...
export async function deactivatePermission(id) {
  const rows = await sequelize.query(SQL.PERMISSION.PERM_DEACTIVATE, {
    replacements: { id: Number(id) },
    type: QueryTypes.SELECT
  });
  return getFirstResult(rows);
...
export async function setPermissionActive(id, active) {
  const rows = await sequelize.query(SQL.PERMISSION.PERM_SET_ACTIVE, {
...
// ---------- User-Permission Operations with Pagination ----------

/**
 * List permissions for a specific user with pagination
 */
export async function listUserPermissions(idUser, active, page = 1, pageSize = 10) {
  const params = prepareStoredProcParams({
  ...
  try {
    const rows = await sequelize.query(SQL.USER_PERMISSION.UP_LIST_BY_USER, {
      replacements: params,
      type: QueryTypes.SELECT
    });
    return processPaginatedResult(rows, page, pageSize);
  } catch (error) {
    console.error('Error in listUserPermissions service:', error);
    throw error;
  }
}

/**
 * List users that have a specific permission with pagination
 */
export async function listUsersByPermission(idPermission, active, page = 1, pageSize = 10) {
  const params = prepareStoredProcParams({
  ...
  try {
    const rows = await sequelize.query(SQL.USER_PERMISSION.UP_LIST_BY_PERM, {
      replacements: params,
      type: QueryTypes.SELECT
    });
    return processPaginatedResult(rows, page, pageSize);
  } catch (error) {
    console.error('Error in listUsersByPermission service:', error);
    throw error;
  }
}

export async function addUserPermission(idUser, idPermission) {
  const rows = await sequelize.query(SQL.USER_PERMISSION.UP_ADD, {
...
export async function activateUserPermission(idUser, idPermission) {
  const rows = await sequelize.query(SQL.USER_PERMISSION.UP_ACTIVATE, {
    replacements: { idUser: Number(idUser), idPermission: Number(idPermission) },
    type: QueryTypes.SELECT
  });
  return getFirstResult(rows);
...
export async function deactivateUserPermission(idUser, idPermission) {
  const rows = await sequelize.query(SQL.USER_PERMISSION.UP_DEACTIVATE, {
    replacements: { idUser: Number(idUser), idPermission: Number(idPermission) },
    type: QueryTypes.SELECT
  });
  return getFirstResult(rows);
...
export async function removeUserPermission(idUser, idPermission, hardDelete = false) {
  const rows = await sequelize.query(SQL.USER_PERMISSION.UP_REMOVE, {
...
export async function setUserPermissionActive(idUser, idPermission, active) {
  const rows = await sequelize.query(SQL.USER_PERMISSION.UP_SET_ACTIVE, {
